...
function c(color, text) {
  if (chaiParam.config.disableColors) return text;
...
function chaiParam(chai, utils) {
  ...
  utils.addMethod(
    ...
    'param',
    function (paramName, functionName) {
  ...
  if (chaiParam.config.improveMessages) {
    ...
    utils.getMessage = function getMessage(obj, args) {
      ...
      msg = msg
        ...
        .replace(/(not) to/, c('green', 'not') + ' to')
        .replace(/to not/, 'to ' + c('green', 'not'))
        .replace(
          /(length )?(below|at least|above|at most|within)(\s*)([0-9]*)/,
          c('green', '$1$2$3$4')
        )
        .replace(
          /(include|truthy|falsy|true|false|undefined|match|satisfy|same members as)/,
          c('green', '$1')
        )
        .replace(
          /(exist|empty|arguments|equal|deeply equal|instance of|be a superset)/,
          c('green', '$1')
        );
      ...
      if (chaiParam.config.showCaller && isNode && fnTarget) {
        // show Caller
        ...
        if (fnTarget) {
          ...
          if (++i < trace.length) {
            ...
            var filePath =
              path.dirname(file) + path.sep + c('blue', path.basename(file));
...
chaiParam.param = function param(val, paramName, functionName, topic) {
  if (!chaiParam._param) {
    throw new Error(
      'You need to add chai-param to chai through `chai.use()` method before using it'
    );
  }
